...
bonds come in multiple flavours we have designed a type class hiearchy to model
the complexity as :
...
an zero rate. The fact that it gives us zero 
rates make them very valuable tools in finance as we may use them to discount 
any cashflow regardless how it was generated. The converse is not true, we 
cannot use a serial's rate to discount a bullet.\\
We present three different term structures graphically in figure \ref{fig:anc}
for ease of exposition. A logarithmic shaped term structure characterizes a
normal market, where the return on an investment increases in some proportion
to the time you have tied up your funds. However the shape depends on 
socio-economic factors and are not discussed further.
...
currently willing to loan. We emphasize the time sensitivity of the term 
